...
namespace Plcway.Communication.Protocols.PLC.Mitsubishi
{
    /// <summary>
    /// 三菱plc客户端
    /// </summary>
    public class MitsubishiClient : SocketBase, IEthernetClient
    {
        ...
        public Result<byte[]> Read(string address, ushort length, bool isBit = false)
        {
            ...
            try
            {
                ...
                var sendResult = new Result<byte[]>();
                switch (_version)
                {
                    case MitsubishiVersion.A_1E:
        ...
        public Result<List<KeyValuePair<string, bool>>> ReadBoolean(string address, ushort readNumber)
        {
            ...
            var dbAddress = decimal.Parse(address[1..]);
        ...
        protected byte[] GetWriteCommand_A_1E(int beginAddress, byte[] typeCode, byte[] data, bool isBit)
        {
            ...
            var command = new byte[12 + data.Length];
            ...
            foreach (var typeChar in typeChars)
            {
                ...
                while (maxAddress >= minAddress)
                {
                    ...
                    readLength = tempMax.DataTypeEnum switch
                    {
                        ...
                        DataTypeEnum.Int32 or DataTypeEnum.UInt32 or DataTypeEnum.Float => tempMax.BeginAddress * 4 + 4 - minAddress * 4,
                        DataTypeEnum.Int64 or DataTypeEnum.UInt64 or DataTypeEnum.Double => tempMax.BeginAddress + 8 - minAddress,
                    ...
                    foreach (var item in tempAddress)
                    {
                        ...
                        {
                            DataTypeEnum.Bool or DataTypeEnum.Byte => throw new Exception("Err BatchRead 未定义类型 -2"),
