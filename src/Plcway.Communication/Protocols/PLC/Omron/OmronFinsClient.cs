...
namespace Plcway.Communication.Protocols.PLC.Omron
{
    /// <summary>
    /// 欧姆龙PLC 客户端
    ...
    {
        private readonly EndianFormat _endianFormat;
        private readonly int _timeout;

        /// <summary>
        /// 基础命令
        /// </summary>
        private readonly byte[] BasicCommand = new byte[]
        {
        ...
        public OmronFinsClient(string ip, int port = 9600, int timeout = 1500, EndianFormat endianFormat = EndianFormat.CDAB)
        {
            ...
            this._endianFormat = endianFormat;
        }
        ...
        public Result<byte[]> Read(string address, ushort length, bool isBit = false, bool setEndian = true)
        {
            ...
            try
            {
                ...
                if (setEndian)
                    result.Value = responseData.ToArray().ByteFormatting(_endianFormat, false);
                else
        ...
        private Result<bool> ReadBoolean(int startAddressInt, int addressInt, byte[] values)
        {
            try
            {
                ...
                return new Result<bool>(BitConverter.ToBoolean(byteArry, 0));
            }
            catch (Exception ex)
            {
        ...
        private Result<short> ReadInt16(int startAddressInt, int addressInt, byte[] values)
        {
            try
            {
                ...
                return new Result<short>(BitConverter.ToInt16(byteArry, 0));
            }
            catch (Exception ex)
            {
        ...
        private Result<ushort> ReadUInt16(int startAddressInt, int addressInt, byte[] values)
        {
            try
            {
                var interval = addressInt - startAddressInt;
                var byteArry = values.Skip(interval * 2).Take(2).Reverse().ToArray();
                return new Result<ushort>(BitConverter.ToUInt16(byteArry, 0));
            }
            catch (Exception ex)
            {
        ...
        private Result<int> ReadInt32(int startAddressInt, int addressInt, byte[] values)
        {
            try
            {
                ...
                var byteArry = values.Skip(interval * 2 * 2 + offset).Take(2 * 2).ToArray().ByteFormatting(_endianFormat, false);
                return new Result<int>(BitConverter.ToInt32(byteArry, 0));
            }
            catch (Exception ex)
            {
        ...
        private Result<uint> ReadUInt32(int startAddressInt, int addressInt, byte[] values)
        {
            try
            {
                ...
                var offset = (addressInt - startAddressInt) % 2 * 2;//取余 乘以2（每个地址16位，占两个字节）
                var byteArry = values.Skip(interval * 2 * 2 + offset).Take(2 * 2).ToArray().ByteFormatting(_endianFormat, false);
                return new Result<uint>(BitConverter.ToUInt32(byteArry, 0));
            }
            catch (Exception ex)
            {
        ...
        private Result<long> ReadInt64(int startAddressInt, int addressInt, byte[] values)
        {
            try
            {
                ...
                var byteArry = values.Skip(interval * 2 * 4 + offset).Take(2 * 4).ToArray().ByteFormatting(_endianFormat, false);
                return new Result<long>(BitConverter.ToInt64(byteArry, 0));
            }
            catch (Exception ex)
            {
        ...
        private Result<ulong> ReadUInt64(int startAddressInt, int addressInt, byte[] values)
        {
            try
            {
                ...
                var offset = (addressInt - startAddressInt) % 4 * 2;
                var byteArry = values.Skip(interval * 2 * 4 + offset).Take(2 * 4).ToArray().ByteFormatting(_endianFormat, false);
                return new Result<ulong>(BitConverter.ToUInt64(byteArry, 0));
            }
            catch (Exception ex)
            {
        ...
        public Result<float> ReadFloat(int beginAddressInt, int addressInt, byte[] values)
        {
            try
            {
                ...
                var byteArry = values.Skip(interval * 2 * 2 + offset).Take(2 * 2).ToArray().ByteFormatting(_endianFormat, false);
                return new Result<float>(BitConverter.ToSingle(byteArry, 0));
            }
            catch (Exception ex)
            {
        ...
        public Result<double> ReadDouble(int beginAddressInt, int addressInt, byte[] values)
        {
            try
            {
                ...
                var byteArry = values.Skip(interval * 2 * 4 + offset).Take(2 * 4).ToArray().ByteFormatting(_endianFormat, false);
                return new Result<double>(BitConverter.ToDouble(byteArry, 0));
            }
            catch (Exception ex)
            {
        ...
        public Result Write(string address, byte[] data, bool isBit = false)
        {
            ...
            try
            {
                data = data.Reverse().ToArray().ByteFormatting(_endianFormat);
                //发送写入信息
                var arg = ConvertArg(address, isBit: isBit);
        ...
        private OmronFinsAddress ConvertArg(string address, DataTypeEnum dataType = DataTypeEnum.None, bool isBit = false)
        {
            ...
            switch (address[0])
            {
                        ...
                        if (isBit)
                        {
                            ...
                            if (address_split.Length > 2)
                            {
                                ...
                                if (addressInfo.BitAddress[2] > 15)
                                    throw new Exception("位地址数据异常"); // 输入的位地址只能在0-15之间
                            }
                        }
                        else
                        {
                ...
                default:
                    throw new Exception("Address解析异常"); // 类型不支持
            }
        ...
        public Result<Dictionary<string, object>> BatchRead(Dictionary<string, DataTypeEnum> addresses, int batchNumber)
        {
            ...
            foreach (var typeChar in typeChars)
            {
                ...
                while (maxAddress >= minAddress)
                {
                    ...
                    readLength = tempMax.DataTypeEnum switch
                    {
                        DataTypeEnum.Bool or DataTypeEnum.Byte => tempMax.BeginAddress + 1 - minAddress,
                        DataTypeEnum.Int16 or DataTypeEnum.UInt16 => tempMax.BeginAddress * 2 + 2 - minAddress * 2,
                        DataTypeEnum.Int32 or DataTypeEnum.UInt32 or DataTypeEnum.Float => tempMax.BeginAddress * 2 + 4 - minAddress * 2,
                        DataTypeEnum.Int64 or DataTypeEnum.UInt64 or DataTypeEnum.Double => tempMax.BeginAddress * 2 + 8 - minAddress * 2,
                    ...
                    {
                        object tempVaue = item.DataTypeEnum switch
                        {
                            DataTypeEnum.Bool => ReadBoolean(minAddress, item.BeginAddress, rValue).Value,
                            DataTypeEnum.Byte => throw new Exception("Err BatchRead 未定义类型 -2"),
                            DataTypeEnum.Int16 => ReadInt16(minAddress, item.BeginAddress, rValue).Value,
                            DataTypeEnum.UInt16 => ReadUInt16(minAddress, item.BeginAddress, rValue).Value,
                            DataTypeEnum.Int32 => ReadInt32(minAddress, item.BeginAddress, rValue).Value,
                            DataTypeEnum.UInt32 => ReadUInt32(minAddress, item.BeginAddress, rValue).Value,
                            DataTypeEnum.Int64 => ReadInt64(minAddress, item.BeginAddress, rValue).Value,
                            DataTypeEnum.UInt64 => ReadUInt64(minAddress, item.BeginAddress, rValue).Value,
                            DataTypeEnum.Float => ReadFloat(minAddress, item.BeginAddress, rValue).Value,
                            DataTypeEnum.Double => ReadDouble(minAddress, item.BeginAddress, rValue).Value,
                            _ => throw new Exception("Err BatchRead 未定义类型 -3"),
                        };
                        result.Value.Add(item.TypeChar + item.BeginAddress.ToString(), tempVaue);
                    }
                    minAddress += readLength / 2;
