...
namespace Plcway.Communication.Protocols.PLC.Siemens
{
    ...
    public class SiemensClient : SocketBase, IEthernetClient
    {
        ...
        /// <param name="ipEndPoint">IP地址和端口号</param>
        /// <param name="timeout">超时时间</param>
        /// <param name="slot">PLC的插槽号</param>
        /// <param name="rack">PLC的机架号</param>
        public SiemensClient(SiemensVersion version, IPEndPoint ipEndPoint, byte slot = 0x00, byte rack = 0x00, int timeout = 1500)
        {
            _version = version;
        ...
        private Result<Dictionary<string, object>> BatchRead(Dictionary<string, DataTypeEnum> addresses)
        {
            ...
            try
            {
                ...
                foreach (var item in args)
                {
                    ...
                    {
                        DataTypeEnum.Bool => BitConverter.ToBoolean(readResut, 0) ? 1 : 0,
                        DataTypeEnum.Byte => readResut[0],
                        DataTypeEnum.Int16 => BitConverter.ToInt16(readResut, 0),
                        DataTypeEnum.UInt16 => BitConverter.ToUInt16(readResut, 0),
                        DataTypeEnum.Int32 => BitConverter.ToInt32(readResut, 0),
                        DataTypeEnum.UInt32 => BitConverter.ToUInt32(readResut, 0),
                        DataTypeEnum.Int64 => BitConverter.ToInt64(readResut, 0),
                        DataTypeEnum.UInt64 => BitConverter.ToUInt64(readResut, 0),
                        DataTypeEnum.Float => BitConverter.ToSingle(readResut, 0),
                        DataTypeEnum.Double => BitConverter.ToDouble(readResut, 0),
                        _ => throw new Exception($"未定义数据类型：{item.DataType}"),
                    };
        ...
        public Result<string> ReadString(string address)
        {
            ...
            var result2 = new Result<string>(readResut1);
            return result2.EndTime();
        }
        ...
        private Result BatchWrite(Dictionary<string, object> addresses)
        {
            ...
            try
            {
                ...
                foreach (var item in addresses)
                {
                    ...
                    tempData = item.Value.GetType().Name switch
                    {
                        "Boolean" => (bool)item.Value ? new List<byte>() { 0x01 } : new List<byte>() { 0x00 },
                        "Byte" => new List<byte>() { (byte)item.Value },
                        "UInt16" => BitConverter.GetBytes((ushort)item.Value).ToList(),
                        "Int16" => BitConverter.GetBytes((short)item.Value).ToList(),
                        "UInt32" => BitConverter.GetBytes((uint)item.Value).ToList(),
                        "Int32" => BitConverter.GetBytes((int)item.Value).ToList(),
                        "UInt64" => BitConverter.GetBytes((ulong)item.Value).ToList(),
                        "Int64" => BitConverter.GetBytes((long)item.Value).ToList(),
                        "Single" => BitConverter.GetBytes((float)item.Value).ToList(),
                        "Double" => BitConverter.GetBytes((double)item.Value).ToList(),
                        _ => throw new Exception($"暂未提供对{item.Value.GetType().Name}类型的写入操作。"),
                    };
                    tempData.Reverse();
        ...
        {
            var writeAddresses = new Dictionary<string, object>
            {
                { address, value }
            };
            ...
            try
            {
                ...
                switch (address[0])
                {
                    ...
                    case 'D':
                        ...
                        if (address[1] == 'B')
                            addressInfo.DbBlock = Convert.ToUInt16(adds[0][2..]);
                        else
                            addressInfo.DbBlock = Convert.ToUInt16(adds[0][1..]);
                        //TODO 
                        addressInfo.BeginAddress = GetBeingAddress(address[(address.IndexOf('.') + 1)..]);
                        break;
                    case 'T':
                ...
                {
                    addressInfo.BeginAddress = GetBeingAddress(address[1..]);
                }
        ...
        {
            return new SiemensWriteAddress(ConvertArg(address))
            {
                WriteData = writeData,
                ReadWriteBit = bit
            };
        ...
        {
            return addresses.Select(t => new SiemensWriteAddress(ConvertArg(t.Key))
            {
                WriteData = t.Value.Key,
                ReadWriteBit = t.Value.Value
            }).ToArray();
        }
        ...
        /// </summary>      
        /// <returns></returns>
        private byte[] GetReadCommand(SiemensAddress[] datas)
        {
        ...
        /// <returns></returns>
        private byte[] GetReadCommand(SiemensAddress data)
        {
        ...
        private byte[] GetWriteCommand(SiemensWriteAddress[] writes)
        {
            ...
            for (int i = 0; i < writes.Length; i++)
            {
                ...
                if (write.WriteData.Length == 1)
                {
                    ...
                    {
                        index += (4 + 2); // fill byte（如果不是最后一个bit，则需要填充一个空数据）
                    }
                }
                else
                {
        ...
        /// <returns></returns>
        private byte[] GetWriteCommand(SiemensWriteAddress write)
        {
